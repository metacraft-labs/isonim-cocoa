#!/usr/bin/env bash
# build-nim-ios.sh — Compile Nim branded app to iOS ARM64 static library.
#
# Uses Nim's --compileOnly mode to generate C/C++ files and a JSON manifest,
# then compiles each file with the system clang targeting the iOS SDK.
#
...
NIM_CACHE="nimcache/ios-app"
OUT_DIR="build"
LIB_NAME="libisonim_app.a"
...
JSON="$NIM_CACHE/libapp_entry.json"
...
echo "==> Checking for isonim_start symbol:"
nm "$OUT_DIR/$LIB_NAME" 2>/dev/null | grep -i "isonim_start" || echo "    WARNING: isonim_start not found"
